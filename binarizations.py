...
class ImageBinarization(Gray):
    def __init__(self, threshold=125):
        self.threshold = threshold
    
    def imageBinar(self, image):
        image = self.grayFill(image)
        imagematrix = imageToMatrix(image)
        ...
        for i in range(height):
            for j in range(width):
                if imagematrix[i, j] > self.threshold:
                    new[i, j] = 255
                else:
                    new[i, j] = 0
...
photos = loadImage('./19.jpg')

ImageBinarization = ImageBinarization()
image = ImageBinarization.imageBinar(photos)
image.show()
